...
const ViewPass = ({ open, onClose, passData }) => {
  ...
  return (
    ...
    >
      <Paper
        className="bg-white p-4 printableArea shadow-md w-full"
      ...
      >
        ...
        <div className="grid grid-cols-3 gap-4 items-start border-2 border-gray-300 rounded-lg shadow-sm">
          <div className="absolute inset-0">
            <IconButton
              onClick={handlePrint}
              className="fixed top-6 left-6 p-2"
            >
              <PrintIcon />
            </IconButton>
          </div>
            ...
            <div className="p-2">
              ...
              <div className="flex">
                <div className="flex-1 flex flex-col space-y-4 p-2">
                  <InfoItem
                    label="Name"
                    value={`${passData?.visitor.first_name} ${passData?.visitor.last_name}`}
                  />
                  <InfoItem
                    label="Type"
                    value={passData?.visitor.visitor_type}
                  />
                  <InfoItem
                    label="Purpose"
                ...
                <div className="flex-1 flex flex-col space-y-4 p-2">
                  ...
                  <InfoItem
                    label="Zones"
                    value={
                      Array.isArray(passData?.zone_names)
                        ? passData.zone_names.join(", ")
                        : "No zones"
                    }
                  />
                </div>
              ...
              <div className="flex justify-left p-2">
                <InfoItem
                  label="Valid-Till"
                  value={new Date(passData?.valid_until).toLocaleString(
                    "en-IN",
                    {
                      year: "numeric",
                      month: "2-digit",
